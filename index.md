...
We can also visualize the results using the
...
![](reference/figures/README-plotres-1.png)
...
2.  **Column-based checks** with `data_columns` in
...
- [`rule()`](https://davzim.github.io/dataverifyr/reference/rule.md) is
  about **values inside rows**.
- [`data_column()`](https://davzim.github.io/dataverifyr/reference/data_column.md)
  is about the **declared structure (schema)** of the dataset.
...
Column checks are attached to the
[`ruleset()`](https://davzim.github.io/dataverifyr/reference/ruleset.md)
via `data_columns`.
...
when the dataset is a
[`dplyr::tbl()`](https://dplyr.tidyverse.org/reference/tbl.html)
connected to an `SQLite` database, the package will automatically choose
`RSQLite`/`DBI`/`dbplyr` for the task. To see which backend
`dataverifyr` would use for a task, you can use `detect_backend(data)`.
...
saw earlier, we can create the rules in R using the
[`rule()`](https://davzim.github.io/dataverifyr/reference/rule.md) and
[`ruleset()`](https://davzim.github.io/dataverifyr/reference/ruleset.md)
functions, there is however, the (in my opinion) preferred option to
separate the code from the rules by writing the rules in a separate yaml
file and reading them into R.
...
![](reference/figures/README-taxi3-1.png)
...
[`filter_fails()`](https://davzim.github.io/dataverifyr/reference/filter_fails.md)
function, which filters a dataset for the failed rule matches
