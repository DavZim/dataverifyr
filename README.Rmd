...
As we saw earlier, we can create the rules in R using the `rules()` and `ruleset()` functions, there is however, the (in my case) preferred option to separate the code from the rules by writing the rules in a separate yaml file and read them into R.
