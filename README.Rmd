...
Then we can create some rules that we want to see if our data conforms to.
Ideally this would be done in the yaml-file directly to separate the execution code from the data, but for now we will do it here in R (replace the `rules` definition with `rules <- read_rules("myrules.yaml")` to load the rules from disk).
...
As we can see from the results, there is one unexpected value, lets quickly investigate
...
```
As we can see, there is probably a data error (a trip distance of 190k miles in 1 minute seems - ehm stellar...).
```{r, echo=FALSE}
...
If this library is not what you are looking for, the following might be good alternatives:
